    ...
    [':+: (cons (first music) (parse-music-duration-sequence (rest music) duration))
          #;(match (second music)
            [(list 'note (list a b) c) (if (< duration (get-musical-duration (second music)))
                       '()
                       (cons ':+: (cons (second music) (parse-music-duration (cons ':+: (rest (rest music))) (- duration (get-musical-duration (second music)))))))]
            [_ (cons (parse-music-duration (second music) duration))])]
    [':=: (cons (first music) (parse-music-duration-parallel (rest music) duration))
          #;(match (second music)
            [(list 'note (list a b) c) (if (< duration (get-musical-duration (second music)))
                       '()
                       (parse-music-duration (cons ':=: (rest (rest music))) duration))]
            [_ (parse-music-duration (second music) duration)])]
...
;returns the same list you passed minus as much of the duration you passed without information loss
