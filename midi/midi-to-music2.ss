...
(define (delta-time->note-durations track-lst tempo)
  (if (not (empty? track-lst))
      (cons (safety-wrapper_temp (add-rests (make-all-note-pairs (first track-lst) 0)) tempo)
            ;(map (lambda (x) (time->note-duration x tempo)) (add-rests (make-all-note-pairs (first track-lst) 0))) ;(make-all-note-pairs (first track-lst) 0))
