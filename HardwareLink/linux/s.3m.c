...
# define FUNCCALL_AGAIN(x) FUNCCALL_AGAIN_once(x)
  ...
  midiList = (SchemeMidi * * ) FUNCCALL(SETUP_XfOrM1(_), malloc (sizeof (SchemeMidi * ) * 100 ) ); 
  q -> size = 0 ; 
  q -> midiList = midiList ; 
...
# define FUNCCALL_AGAIN(x) FUNCCALL_AGAIN_each(x)
  ...
  client = NULLED_OUT ; 
  portName = NULLED_OUT ; 
  inPort = (MIDIPortRef * ) FUNCCALL(SETUP_XfOrM11(_), malloc (sizeof (MIDIPortRef ) ) ); 
  client = (MIDIClientRef * ) FUNCCALL(SETUP_XfOrM11(_), malloc (sizeof (MIDIClientRef ) ) ); 
  nSrcs = FUNCCALL(SETUP_XfOrM11(_), MIDIGetNumberOfSources () ); 
  ...
  for (iSrc = 0 ; iSrc < nSrcs ; ++ iSrc ) {
    MIDIEndpointRef src ; 
    void * srcConnRefCon ; 
    BLOCK_SETUP((PUSH(src, 0+XfOrM11_COUNT), PUSH(srcConnRefCon, 1+XfOrM11_COUNT)));
#   define XfOrM13_COUNT (2+XfOrM11_COUNT)
#   define SETUP_XfOrM13(x) SETUP(XfOrM13_COUNT)
    src = NULLED_OUT ; 
    srcConnRefCon = NULLED_OUT ; 
    src = FUNCCALL(SETUP_XfOrM13(_), MIDIGetSource (iSrc ) ); 
    srcConnRefCon = src ; 
    FUNCCALL(SETUP_XfOrM13(_), MIDIPortConnectSource (* inPort , src , srcConnRefCon ) ); 
  }
  RET_VALUE_START (0 - nSrcs ) RET_VALUE_END ; 
# undef BLOCK_SETUP
# undef FUNCCALL
# undef FUNCCALL_EMPTY
# undef FUNCCALL_AGAIN
