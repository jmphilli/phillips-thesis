...
# define FUNCCALL_AGAIN(x) FUNCCALL_AGAIN_each(x)
  ...
  client = NULLED_OUT ; 
  portName = NULLED_OUT ; 
  inPort = (MIDIPortRef * ) FUNCCALL(SETUP_XfOrM2(_), malloc (sizeof (MIDIPortRef ) ) ); 
  client = (MIDIClientRef * ) FUNCCALL(SETUP_XfOrM2(_), malloc (sizeof (MIDIClientRef ) ) ); 
  nSrcs = FUNCCALL(SETUP_XfOrM2(_), MIDIGetNumberOfSources () ); 
  ...
  for (iSrc = 0 ; iSrc < nSrcs ; ++ iSrc ) {
    MIDIEndpointRef src ; 
    void * srcConnRefCon ; 
    BLOCK_SETUP((PUSH(src, 0+XfOrM2_COUNT), PUSH(srcConnRefCon, 1+XfOrM2_COUNT)));
#   define XfOrM4_COUNT (2+XfOrM2_COUNT)
#   define SETUP_XfOrM4(x) SETUP(XfOrM4_COUNT)
    src = NULLED_OUT ; 
    srcConnRefCon = NULLED_OUT ; 
    src = FUNCCALL(SETUP_XfOrM4(_), MIDIGetSource (iSrc ) ); 
    srcConnRefCon = src ; 
    FUNCCALL(SETUP_XfOrM4(_), MIDIPortConnectSource (* inPort , src , srcConnRefCon ) ); 
  }
...
SchemeMidi * MIDIPacketToSchemeMidi (MIDIPacket * packet ) {
  SchemeMidi * data ; 
  DECL_RET_SAVE (SchemeMidi * ) PREPARE_VAR_STACK_ONCE(2);
  BLOCK_SETUP_TOP((PUSH(data, 0), PUSH(packet, 1)));
...
# define FUNCCALL_AGAIN(x) FUNCCALL_AGAIN_once(x)
  data = NULLED_OUT ; 
  data = FUNCCALL(SETUP_XfOrM5(_), malloc (sizeof (SchemeMidi * ) ) ); 
...
# define FUNCCALL_AGAIN(x) FUNCCALL_AGAIN_once(x)
  mod_env = NULLED_OUT ; 
  mod_env = (__funcarg126 = FUNCCALL(SETUP_XfOrM7(_), scheme_intern_symbol ("SchemeMidi" ) ), FUNCCALL_AGAIN(scheme_primitive_module (__funcarg126 , env ) )) ; 
  FUNCCALL_EMPTY(scheme_finish_primitive_module (mod_env ) ); 
  q = FUNCCALL_EMPTY(newQueue () ); 
  RET_VALUE_EMPTY_START (FUNCCALL_EMPTY(scheme_make_utf8_string ("ok" ) )) RET_VALUE_EMPTY_END ; 
# undef BLOCK_SETUP
# undef FUNCCALL
# undef FUNCCALL_EMPTY
# undef FUNCCALL_AGAIN
...
Queue * newQueue () {
  ...
  BLOCK_SETUP_TOP((PUSH(midiList, 0), PUSH(q, 1)));
...
# define FUNCCALL_AGAIN(x) FUNCCALL_AGAIN_once(x)
  q = NULLED_OUT ; 
  midiList = NULLED_OUT ; 
  q = (Queue * ) FUNCCALL(SETUP_XfOrM10(_), malloc (sizeof (Queue ) ) ); 
  midiList = (SchemeMidi * * ) FUNCCALL(SETUP_XfOrM10(_), malloc (sizeof (SchemeMidi * ) * 100 ) ); 
  q -> size = 0 ; 
  q -> midiList = midiList ; 
